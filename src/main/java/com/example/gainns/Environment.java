...
public class Environment extends Application {
    ...
    void handleMouse(Node node) {
    	node.setOnMouseReleased(me -> {
    		if ((node == this.srRotate) && (this.selectedElement.isCharMenuShowing() == shapesMenu.isCharMenuShowing())) { //check if right menu selected
    			setRotate(me.getX(), me.getY(), true);
    			this.srRotate.setCursor(Cursor.OPEN_HAND);
    		}
    	});
        node.setOnMousePressed(me -> {
            if (this.selectedElement.isCharMenuShowing() == shapesMenu.isCharMenuShowing()) { //check if right menu selected
